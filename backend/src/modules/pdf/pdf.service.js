...
const generateChitInvoicePDF = async (paymentId, options = {}) => {
  ...
  const { ChitFundPayment, ChitSubscriber, ChitScheme, Customer, GoldRate } = require('../../models');
  const sharedPdfService = require('../../shared/utils/pdf.service');
  ...
  const paymentHistory = await ChitFundPayment.findAll({
    where: { chitSubscriberId: subscriber.id, status: 'SUCCESS' },
  ...
  const buffer = await sharedPdfService.generateChitInvoice(payment, subscriber, scheme, customer, paymentHistory, currentGoldRate);
